...
const Modal = ({ children, containerClasses, open, onClose }: ModalProps) => {
  ...
  if (typeof window === "object" && isClient) {
    return (
      ...
      createPortal(
        <div
          className={`smooth-transition fixed inset-0 z-[999999999] flex items-center justify-center bg-black/40 backdrop-blur-sm will-change-["opacity"] ${containerClasses}  ${
            open
